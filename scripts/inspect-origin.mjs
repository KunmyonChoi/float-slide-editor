import puppeteer from 'puppeteer'
import { readFileSync } from 'fs'

const SOURCE_HTML = '/home/kunmyon/Slide-editor/slides/AI DC Study Book_slides.html'
const CANVAS_W = 1280, CANVAS_H = 720, SLIDE_INDEX = 0

function makeSlideVisible(html, index) {
  let result = html
  result = result.replace(/<script>[\s\S]*?window\.hasSlideNav[\s\S]*?<\/script>/m, '')
  result = result.replace(/class="slide active"/g, 'class="slide"')
  result = result.replace(/class="slide active /g, 'class="slide ')
  const injection = `<style>.slide{display:none!important}.deck>.slide:nth-child(${index+1}),body>.slide:nth-child(${index+1}){display:flex!important}</style>`
  result = result.replace('</head>', injection + '\n</head>')
  result = result.replace('<body>', `<body style="width:${CANVAS_W}px;height:${CANVAS_H}px;overflow:hidden;margin:0;padding:0;">`)
  return result
}

const browser = await puppeteer.launch({ headless: true, args: ['--no-sandbox','--disable-setuid-sandbox','--disable-web-security'], defaultViewport: { width: CANVAS_W, height: CANVAS_H, deviceScaleFactor: 2 } })
const page = await browser.newPage()
const sourceHtml = readFileSync(SOURCE_HTML, 'utf-8')
await page.setContent(makeSlideVisible(sourceHtml, SLIDE_INDEX), { waitUntil: 'networkidle0', timeout: 20000 })
await page.evaluate(() => document.fonts.ready)
await new Promise(r => setTimeout(r, 300))

// Find elements with text containing "AI Data Center"
const result = await page.evaluate(() => {
  const allText = [...document.querySelectorAll('*')].filter(el => {
    const directText = [...el.childNodes].filter(n => n.nodeType === 3).map(n => n.textContent.trim()).join('')
    return directText.includes('AI Data Center') || directText.includes('AI DC') || directText.includes('기술 심층')
  })
  return allText.map(el => {
    const cs = window.getComputedStyle(el)
    const rect = el.getBoundingClientRect()
    return {
      tag: el.tagName,
      text: el.textContent.trim().substring(0, 50),
      rect: { x: Math.round(rect.x), y: Math.round(rect.y), w: Math.round(rect.width), h: Math.round(rect.height) },
      textShadow: cs.textShadow,
      backgroundImage: cs.backgroundImage?.substring(0, 80),
      webkitTextFillColor: cs.webkitTextFillColor,
      webkitBackgroundClip: cs.webkitBackgroundClip,
      backgroundClip: cs.backgroundClip,
      color: cs.color,
      fontSize: cs.fontSize,
      fontWeight: cs.fontWeight
    }
  })
})

result.forEach(r => {
  console.log(`[${r.tag}] "${r.text}"`)
  console.log(`  rect: x=${r.rect.x} y=${r.rect.y} w=${r.rect.w} h=${r.rect.h}`)
  if (r.textShadow && r.textShadow !== 'none') console.log(`  textShadow: ${r.textShadow}`)
  if (r.backgroundImage && r.backgroundImage !== 'none') console.log(`  bgImage: ${r.backgroundImage}`)
  if (r.webkitTextFillColor && r.webkitTextFillColor !== 'rgba(0, 0, 0, 0)') console.log(`  webkitFillColor: ${r.webkitTextFillColor}`)
  if (r.webkitBackgroundClip && r.webkitBackgroundClip !== 'border-box') console.log(`  webkitBgClip: ${r.webkitBackgroundClip}`)
  console.log(`  color: ${r.color} fs=${r.fontSize} fw=${r.fontWeight}`)
})
await browser.close()
