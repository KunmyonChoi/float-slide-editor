...
const elements = await page.evaluate((cw, ch) => {
  ...
  return elements.map(e => ({
    ...
    styleKeys: Object.keys(e.styles || {}),
    bgColor: e.styles?.backgroundColor,
    bgShort: (e.styles?.background || '').substring(0, 80),
    outerHtml: e.outerHTML?.substring(0, 60)
  }))
}, CANVAS_W, CANVAS_H)
...
console.log('\nAll elements:')
elements.forEach(e => {
  console.log(`[${e.id}/${e.type}] x=${e.x} y=${e.y} w=${e.w} h=${e.h}`)
  if (e.bgShort) console.log(`  bg: ${e.bgShort}`)
  if (e.outerHtml) console.log(`  html: ${e.outerHtml}`)
})
await browser.close()
