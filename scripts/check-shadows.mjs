...
// Find all elements with box-shadow  
const result = await page.evaluate(() => {
  const allEls = [...document.querySelectorAll('*')]
  const withShadow = allEls.filter(el => {
    const cs = window.getComputedStyle(el)
    const shadow = cs.boxShadow
    return shadow && shadow !== 'none'
  })
  return withShadow.map(el => {
    const cs = window.getComputedStyle(el)
...
result.forEach(r => {
  if (r.rect.w > 0) {
    console.log(`[${r.tag}] class="${r.class}" x=${r.rect.x} y=${r.rect.y} w=${r.rect.w} h=${r.rect.h}`)
    console.log(`  shadow: ${r.boxShadow}`)
  }
})
await browser.close()
