...
않으므로, 글자가 상자를 뚫고 나가도 아무도 막지 않는다. 실제 덱에서 본문 블록이 상자 바닥을
4~14px 넘긴 채 나왔다 — 줄바꿈 때문이 아니라 **선언한 좌표가 이미 넘쳐 있었다.**

```
글자 top    ≥ 상자 top + 여백
글자 top + height ≤ 상자 top + 상자 height − 여백
여백 = 상자 높이의 10% (최소 10px, 최대 20px). 좌우는 보통 더 넉넉히(24~44px).
```

- 텍스트 height는 **실제 줄 수 × 줄높이**로 잡는다. 넉넉하게 잡아 두면 그 값이 그대로 상자를
  넘는다(높이를 키워도 글자가 늘지 않는다 — 남는 공간은 상자가 갖게 할 것).
- 한 상자 안에 여러 블록(라벨+본문)을 넣을 때는 `여백 + Σ블록높이 + Σ간격 + 여백 ≤ 상자 높이`를
  먼저 확인하고 나서 좌표를 적는다.
- 줄바꿈이 예상보다 늘어나면(WRAP) 이 계산이 깨진다 — 긴 문장은 폭·글자 크기부터 조정할 것.
- 검수 스크립트의 **PAD** 검사가 이 관계를 본다(넘치면 문제, 여백이 좁으면 경고).
...
- **카드 높이를 눈대중으로 잡고 그 안에 글자를 얹기** → 절대 위치 형제라 아무도 막아 주지 않는다.
  위의 "상자 안 글자에는 여백을 남긴다"로 검산할 것.
