...
const PATTERN_RULES = [
  ...
  {
    pattern: 'unintended_wrap',
    issueType: 'unintended_wrap',
    match: () => true,
    description: '텍스트 박스 너비가 좁아 의도치 않은 줄바꿈이 발생함',
    suggestedFix: 'extractFlatElements()에서 텍스트 요소의 width가 원본 레이아웃보다 좁게 계산되는지 확인. getBoundingClientRect()가 CSS box 모델을 정확히 반영하는지 검증.',
    relevantCode: 'src/core/FlatExtractor.js → buildFlatElement(), tryMergeContainerText() width 계산',
  },
  {
    pattern: 'out_of_bounds',
    issueType: 'out_of_bounds',
    match: () => true,
    description: '요소가 캔버스 영역 밖에 위치하여 보이지 않음',
    suggestedFix: 'extractFlatElements()의 getBoundingClientRect() 좌표가 body 기준인지 확인. display:none 슬라이드의 요소가 추출되지 않는지 확인.',
    relevantCode: 'src/core/FlatExtractor.js → extractFlatElements() rect 좌표',
  },
  {
    pattern: 'element_overlap',
    issueType: 'overlap',
    match: () => true,
    description: '텍스트 요소가 다른 텍스트 요소와 과도하게 겹침',
    suggestedFix: 'z-index 재정렬 로직 확인. 원본에서 겹치지 않는 요소가 flat에서 겹치면 위치 계산 오류.',
    relevantCode: 'src/core/FlatExtractor.js → buildFlatElement(), getEffectiveZIndex()',
  },
  {
    pattern: 'size_correction_large',
    issueType: 'size_correction',
    match: () => true,
    description: '텍스트 너비 보정이 20px 이상 발생 — 폰트 메트릭 불일치 가능',
    suggestedFix: 'flat HTML에 원본과 동일한 폰트 @import가 포함되는지 확인. buildFlatElement()의 nowrap 측정 로직 검증.',
    relevantCode: 'src/core/FlatExtractor.js → buildFlatElement() width 보정, src/core/FlatExporter.js → fontImports',
  },
  {
