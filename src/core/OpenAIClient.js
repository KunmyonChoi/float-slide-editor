...
const ADVISER_SYSTEM = `You are an expert presentation coach. You are given a slide deck (each slide's text), plus the presentation's purpose and audience. Evaluate the deck and give concrete, actionable advice.

Output ONLY a single valid JSON object with exactly these keys:
{
  "overall": { "summary": "1-3 sentence overall assessment", "strengths": ["short bullet", ...], "improvements": ["short, actionable bullet", ...] },
  "flow": "assessment of the narrative flow/structure across slides, with concrete suggestions (reorder, add, merge, cut)",
  "slides": [ { "index": 1, "comment": "what works / what to fix on this slide", "suggestions": ["short actionable item", ...] } ]
}

Rules:
- Respond in Korean. Be SPECIFIC to the actual content — avoid generic advice.
- Tailor everything to the stated purpose and audience.
- "slides" MUST include an entry for EVERY slide index provided, in order.
- Keep each string concise. Empty slides: note that they look empty.`

/**
 * 슬라이드 덱 + 발표 목적/청중 → 어드바이저 평가(JSON).
 * @param {Array<{index:number, text:string}>} slides 슬라이드별 평문 텍스트(1-based index)
 * @param {{ purpose?: string, audience?: string, model?: string, signal?: AbortSignal }} [opts]
 * @returns {Promise<object>} { overall:{summary,strengths,improvements}, flow, slides:[{index,comment,suggestions}] }
 */
export async function advisePresentation(slides, { purpose, audience, model, signal } = {}) {
  if (!slides || slides.length === 0) throw new Error('분석할 슬라이드가 없습니다.')
  const deck = slides
    .map(s => `[Slide ${s.index}]\n${(s.text || '').trim() || '(빈 슬라이드)'}`)
    .join('\n\n')
  const user = `발표 목적: ${(purpose || '').trim() || '(미지정)'}\n청중: ${(audience || '').trim() || '(미지정)'}\n슬라이드 수: ${slides.length}\n\n=== 슬라이드 내용 ===\n${deck}`
  ...
  let parsed
  try { parsed = JSON.parse(raw) } catch { throw new Error('어드바이저 응답을 해석하지 못했습니다.') }
  return parsed
}

const APPLY_SYSTEM = `You are a presentation editor. You are given one slide's text elements (each with an id and its current text) plus improvement advice. Rewrite ONLY the text content to address the advice, keeping the SAME elements and overall structure. Do NOT add or remove elements.

Output ONLY a single valid JSON object:
{ "updates": [ { "id": "<element id>", "text": "<improved text>" } ], "note": "advice that cannot be applied by text alone (structure/layout), or empty" }

Rules:
- Respond in the SAME language as each original text (Korean stays Korean). Do not translate.
- Include ONLY elements whose text you actually changed.
- Stay faithful to the slide's meaning; do not invent facts or numbers.
- Keep it concise. Use \\n for line breaks where appropriate.`

/**
 * 한 슬라이드의 텍스트 요소들을 조언에 맞게 재작성(텍스트 내용만).
 * @param {Array<{id:string, text:string}>} textElements 현재 텍스트 요소(평문)
 * @param {{ comment?: string, suggestions?: string[], purpose?: string, audience?: string, model?: string, signal?: AbortSignal }} [opts]
 * @returns {Promise<{ updates: Array<{id:string, text:string}>, note?: string }>}
 */
export async function applyAdviceToSlide(textElements, { comment, suggestions, purpose, audience, model, signal } = {}) {
  if (!textElements || textElements.length === 0) throw new Error('편집할 텍스트가 없습니다.')
  const els = textElements.map(e => `[id:${e.id}]\n${e.text}`).join('\n\n')
  const advice = [comment, ...(suggestions || [])].filter(Boolean).join('\n- ')
  const user = `발표 목적: ${(purpose || '').trim() || '(미지정)'}\n청중: ${(audience || '').trim() || '(미지정)'}\n\n조언:\n- ${advice || '(없음)'}\n\n=== 텍스트 요소 ===\n${els}`
  const raw = await chat({
    system: APPLY_SYSTEM,
    user,
  ...
  let parsed
  try { parsed = JSON.parse(raw) } catch { throw new Error('적용 응답을 해석하지 못했습니다.') }
  if (!Array.isArray(parsed.updates)) parsed.updates = []
  return parsed
}

/**
 * 한 슬라이드를 조언에 맞게 **재구성한 HTML 한 장**으로 생성(레이아웃·구성 변경 가능).
 * 반환 HTML은 캔버스 크기에 맞는 자기완결 문서 → 임시 렌더 후 flat 추출에 사용한다.
 * @param {{ text:string }} slide  현재 슬라이드 텍스트
 * @param {{ comment?:string, suggestions?:string[], purpose?:string, audience?:string, canvasSize:{w:number,h:number}, model?:string, signal?:AbortSignal }} opts
 * @returns {Promise<string>} HTML 문서 문자열
 */
export async function regenerateSlideHtml(slide, { comment, suggestions, purpose, audience, canvasSize, model, signal } = {}) {
  const w = canvasSize?.w || 1280
  const h = canvasSize?.h || 720
  const system = `You are an expert slide designer. Redesign ONE presentation slide as a self-contained HTML document that implements the given improvement advice, keeping the slide's meaning.

Output ONLY raw HTML (NO markdown code fences, no commentary).
Requirements:
- A complete document: <html><head><meta charset="utf-8"><style>…</style></head><body>…</body></html>.
- <body> MUST be exactly ${w}px wide and ${h}px tall, with margin:0, overflow:hidden, position:relative, box-sizing:border-box. It represents the whole slide.
- Clean, modern presentation design: clear hierarchy, generous spacing, readable font sizes, a cohesive color palette. Use flexbox/grid or absolute positioning.
- Keep ALL text in the SAME language as the original (Korean stays Korean). Do NOT translate. Do NOT invent facts or numbers.
- Apply the advice: improve layout/structure/emphasis, FIX overflow so all text fits within ${w}x${h}, add simple visual aids (cards, dividers, badges, unicode/emoji icons, simple CSS charts) where helpful.
- Self-contained: inline <style> only. NO external images or scripts. A Google Fonts <link> for a web font is allowed.
- Ensure NOTHING overflows the ${w}x${h} body.`
  const advice = [comment, ...(suggestions || [])].filter(Boolean).join('\n- ')
  const user = `Canvas size: ${w}x${h}\n발표 목적: ${(purpose || '').trim() || '(미지정)'}\n청중: ${(audience || '').trim() || '(미지정)'}\n\n개선 조언:\n- ${advice || '(전반적으로 더 깔끔하고 명확하게)'}\n\n=== 현재 슬라이드 텍스트 ===\n${(slide?.text || '').trim() || '(빈 슬라이드)'}`
  let html = await chat({ system, user, model, temperature: 0.6, signal })
  // 혹시 코드펜스로 감싸 오면 제거
