...
// 톤 지시(instructions)는 결과가 불안정해 걷어냈다 — 같은 문구로도 억양·속도가 요청마다 달라져
// 나레이션이 슬라이드마다 튀었다. 옛 사용자의 저장값이 남아 조용히 섞이지 않도록 키까지 지운다.
try { localStorage.removeItem('openai-tts-instructions') } catch { /* localStorage 비활성 무시 */ }
