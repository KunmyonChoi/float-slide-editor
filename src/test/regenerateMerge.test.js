...
describe('buildRegeneratedCache — 재생성 시 flat-only 페이지 보존', () => {
  const snap = [
    { htmlSlideIndex: 0, entry: { tag: 'html0-old' } },
    { htmlSlideIndex: null, entry: { tag: 'FLAT-ONLY' } },
    { htmlSlideIndex: 1, entry: { tag: 'html1-old' } },
    { htmlSlideIndex: 2, entry: { tag: 'html2-old-removed' } },
  ]
  const fresh = { 0: { tag: 'fresh0' }, 1: { tag: 'fresh1' }, 3: { tag: 'fresh3-new' } }
  ...
  it('원래 순서를 유지하며 html-backed는 새 데이터로 교체', () => {
    ...
    expect(cache['0-0'].htmlSlideIndex).toBe(0)
    expect(cache['2-0'].tag).toBe('fresh1')
    expect(cache['2-0'].htmlSlideIndex).toBe(1)
  })
  ...
  it('사라진 HTML 슬라이드(2)는 제외', () => {
    expect(keys.some(k => cache[k].htmlSlideIndex === 2)).toBe(false)
  })

  it('스냅샷에 없던 새 HTML 슬라이드(3)는 끝에 추가', () => {
    const last = keys[keys.length - 1]
    expect(cache[last].tag).toBe('fresh3-new')
    expect(cache[last].htmlSlideIndex).toBe(3)
  })
